...
| `APPLE_PHOTOS_MCP_STALE_WHILE_REVALIDATE` | unset (**off**) | Set to `1` to serve the previously parsed library immediately when Photos.sqlite has changed, and re-parse on a background thread instead of blocking the request. The persistent sidecar re-validates its parsed `PhotosDB` against `Photos.sqlite`'s mtime on every call, so **any** write to the library — a phone sync, an edit, Photos.app housekeeping — makes the next request pay a full re-parse; on a 15 GB library that is minutes. With this set, that request answers from the last good parse and the refresh lands behind it. The trade is freshness: a result may predate an import by one refresh cycle, and responses carry `dbStale: true` while that is the case. Peak memory roughly doubles during a refresh, since both parses are briefly resident. Worth setting only on libraries big enough for the parse to dominate. |
