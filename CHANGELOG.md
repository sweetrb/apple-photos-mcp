...
- **`persistent-sidecar` integration flake.** The `expected 3 to be 4` failure that intermittently blocked unrelated PRs was this same dropped notification, not a slow one. An earlier attempt widened the assertion's poll from 10s to 30s; that did not help — a 30s budget failed identically on `main` — because a discarded message never arrives no matter how long you wait. With the ordering fixed above, the test asserts the notification count synchronously and the poll is gone, so a regression fails loudly instead of being masked by a longer wait.
