...
export function registerResourcesAndPrompts(server, manager) {
    ...
    server.resource("photo", new ResourceTemplate("photos://photo/{uuid}", { list: undefined }), (uri, variables) => {
        try {
    ...
    // --- Prompts ---
    server.prompt("find-photos", "Find photos matching criteria and summarize them", { criteria: z.string().describe("What photos to look for (people, dates, keywords, etc.)") }, ({ criteria }) => ({
        messages: [
            {
                role: "user",
                content: {
    ...
    server.prompt("export-photos", "Find photos matching criteria and export them to a destination", {
